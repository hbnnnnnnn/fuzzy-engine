...
DB_PATH="${DB_PATH:-${NEXUS_ROOT}/Nexus-Gen/mrag-db}"
...
for f in "${DB_PATH}/image.faiss" "${DB_PATH}/text.faiss" "${DB_PATH}/metadata.json"; do
    if [ -f "$f" ]; then
...
# ── Load metadata ─────────────────────────────────────────────────────────────
_log("META", f"loading metadata  →  {DB_PATH}/metadata.json")
t0 = time.time()
