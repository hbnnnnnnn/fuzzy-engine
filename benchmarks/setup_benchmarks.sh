...
# ============================================================
echo ""
echo ">>> [2/5] Setting up DrawBench prompts"
echo "------------------------------------------------------------"
...
# ============================================================
echo ""
echo ">>> [3/5] Setting up T2I-CompBench"
echo "------------------------------------------------------------"
...
# Use re.DOTALL so the pattern matches across multi-line / type-annotated signatures.
patched = re.sub(
    r'(def _check_cuda_version\b.*?:)',
    r'\1\n    return',
    src,
    count=1,
    flags=re.DOTALL
...
if patched == src:
    sys.exit('ERROR: Could not patch _check_cuda_version — regex did not match. Check PyTorch version.')
...
# Use mim (openmim) to fetch the pre-built wheel for the installed PyTorch + CUDA
# combo instead of compiling from source (which fails against PyTorch 2.x C++ APIs).
python3 -m mim install mmcv-full==1.7.2
