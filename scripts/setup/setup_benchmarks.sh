...
#SBATCH --output=logs/setup_benchmarks_%j.out
#SBATCH --error=logs/setup_benchmarks_%j.err
...
# ---------------------------------------------------------------------------
INSTALL_DIR="${INSTALL_DIR:-/media02/nthuy/ndbao/ndbao_hbngoc}"
export INSTALL_DIR
CONDA_ENV_TIFA="tifa"
CONDA_ENV_T2ICOMP="t2icomp"
CONDA_BASE="${CONDA_BASE:-/media02/nthuy/miniconda3}"
...
echo "Installing fairseq (pre-install to avoid broken PyPI sdist)..."
# fairseq's PyPI source dist is missing fairseq/version.txt, causing build
# failure. Install directly from GitHub which has the file.
pip install --quiet \
    "git+https://github.com/facebookresearch/fairseq.git@main#egg=fairseq"

echo "Installing TIFA..."
pip install --quiet --no-deps -e "${INSTALL_DIR}/tifa"
...
out = os.path.join(os.environ.get("DRAWBENCH_DIR", "/media02/nthuy/ndbao/ndbao_hbngoc/drawbench"),
...
dest_dir = os.path.join(os.environ.get("INSTALL_DIR", "/media02/nthuy/ndbao/ndbao_hbngoc"),
...
echo ""
echo "--- TIFA v1.0 ---"
echo "  source ${CONDA_BASE}/bin/activate && conda activate ${CONDA_ENV_TIFA}"
...
echo "  source ${CONDA_BASE}/bin/activate && conda activate ${CONDA_ENV_T2ICOMP}"
