...
elif [ -d "mrag-db-orgcap" ]; then
    MRAG_DB_PATH="mrag-db-orgcap"
elif [ -d "mrag-db_orgcap" ]; then
    MRAG_DB_PATH="mrag-db_orgcap"
else
    MRAG_DB_PATH="mrag-db"
fi

if [ ! -f "image_generation_mrag.py" ]; then
    echo "❌ Missing required script: image_generation_mrag.py"
    echo "   This controlled script depends on a legacy MRAG generation entrypoint"
    echo "   that is not present in this checkout."
    echo "   Use test_rag_patch_controlled.sh for checkpoint validation, or restore"
    echo "   image_generation_mrag.py before running this script."
    exit 2
fi
